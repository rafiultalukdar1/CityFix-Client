...
const AssignedIssues = () => {
    ...
    const { data: issues = [], isLoading, refetch } = useQuery({
        ...
        queryFn: async () => {
            ...
            if (priorityFilter) data = data.filter(i =>
    ...
    return (
        <>
            <title>CityFix - Assigned Issues</title>
            <div className="px-3 py-12 max-w-[1500px] mx-auto">
                ...
                <div className="flex gap-2.5 mt-6 lg:mt-10 max-w-[650px]">
                ...
                <div className="border border-[#219e64] rounded-xl overflow-x-scroll mt-5 md:mt-8">
                                    ...
                                    <td className="p-4 text-center flex gap-2.5 items-center">
                                        {issue.status !== 'closed' && (
                                            <select
                                                ...
                                                defaultValue=""
                                                className="px-[15px] py-2 text-[#464545] text-[15px] outline-0 border border-[#219e648f] focus:border-[#219E64] bg-[#F3F3F3] rounded dark:text-[#F3F3F3] dark:bg-gray-900 dark:border-[#219E6480] dark:focus:border-[#219E64]"
                                            >
                                                <option value="" disabled> Change Status </option>
                                                {staffStatusOptions.map(s => {
                                                    const isDisabled = s === issue.status || issue.timeline?.some(t => t.status === s)
                                                    return (
                                                        <option
                                                            className={`capitalize ${isDisabled ? 'cursor-not-allowed hidden' : 'cursor-pointer'}`}
                                                            key={s}
                                                            value={s}
                                                            disabled={isDisabled}
                                        ...
                                        <Link to={`/issue-details/${issue._id}`} className="inline-flex items-center justify-center px-2.5 py-2 text-[18px] font-medium bg-[#219E64] hover:bg-[#168652] text-white transition rounded" >
                                            <FaEye />
                                        </Link>
                                    </td>
                                </tr>
                    ...
                    {issues.length === 0 && <p className="text-center py-10 text-gray-500">No assigned issues found</p>}
                </div>
            </div>
        </>
    )
